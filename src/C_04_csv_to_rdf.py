...
def convert(
    ...
    for row in read_csv_dicts(entities_path):
        ...
        if url:
            ...
            ttl.append(emit_literal(subj, p_source, url, dtype="xsd:anyURI"))
    ...
    for row in read_csv_dicts(relations_path):
        ...
        if extras:
            ...
            for k, v in extras.items():
                ...
                elif plocal == "sourceUrl":
                    ttl.append(f"   {pextra} \"{ttl_escape_literal(v)}\"^^xsd:anyURI ;\n")
                else:
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--entities", type=Path, default=Path("data/entities_clean.csv"), help="Path to entities CSV.")
    p.add_argument("--relations", type=Path, default=Path("data/relations_clean.csv"), help="Path to relations CSV.")
